...
class AgentPPO(AgentBase):
    ...
    def select_action_continues(self, state) -> tuple:
    ...
    def select_action_discrete(self, state) -> tuple:
        states = torch.as_tensor((state,), dtype=torch.float32, device=self.device).detach_()
        actions = self.act(states)
        if rd.rand() < self.explore_rate:  # epsilon-greedy
            action = self.softmax(actions)[0]
    ...
    def select_action(self, states) -> tuple:
        if not self.if_discrete:
            return self.select_action_continues(states)
        else:
            return self.select_action_discrete(states)
    ...
    def explore_env(self, env, buffer, target_step, reward_scale, gamma) -> int:
        ...
        while actual_step < target_step:
            ...
            for _ in range(env.max_step):
                ...
                if not self.if_discrete: action = np.tanh(action)
...
class ReplayBuffer:
    def __init__(self, max_len, state_dim:list, action_dim, if_on_policy, if_gpu):
