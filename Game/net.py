...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        # layer_norm(self.fc[-1], std=0.1)  # output layer for action
...
class ActorSAC(nn.Module):
    ...
    def get_action_logprob(self, state):
        ...
        a_std_log = self.net_a_std(t_tmp).clamp(-20, 2) # 在深度学习里， 一个系数(coff) 在 1e-6 到 6 之间的变化，能保证梯度正常
        a_std = a_std_log.exp()

        """add noise to action in stochastic policy"""
        noise = torch.randn_like(a_avg, requires_grad=True)  # \xi
        # action = tanh(\mu(s)+\delta * \xi)
        ...
        # log(\pi(a|s)) = log(\pi(a|s)) - \sum(log(1 - tanh^2(\mu)))
